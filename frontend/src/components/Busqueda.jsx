...
function Busqueda() {
  ...
  const procesarVenta = async () => {
    if (carrito.length === 0) {
      alert('El carrito está vacío. Agregue productos para procesar la venta.')
      return
    }
    ...
    const ventaPayload = {
      ...
      // metodo_pago: 'Efectivo' // Se puede añadir un selector de método de pago en el futuro
    }

    try {
      const response = await fetch('/api/inventarios/sell', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        buscarProductos() // Refresh product list to reflect updated stock
      } else {
                                  ...
                                  <div className="grid grid-cols-2 gap-4 text-sm">
                                    ...
                                    <div>
                                      ...
                                      <span className={`font-medium ${
                                        farmacia.total > 0 ? 'text-green-600' : 'text-red-600'
                                      }`}>
                                        {farmacia.total > 0 ? `${farmacia.total} unidades` : 'Sin stock'}
                                      </span>
                                    </div>
                                    {farmacia.descuento_diario_porcentaje > 0 && (
                                      <div>
                                        <span className="text-gray-600">Desc. Diario: </span>
                ...
                {carrito.length === 0 ? (
                  ...
                  <>
                    ...
                    <div className="border-t pt-4">
                      ...
                      <Button onClick={procesarVenta} className="w-full">
                        Procesar Venta
                      </Button>
                    </div>
                  </>
